...
test_tests =
    ...
    [        
        ...
        , testCase "test Example Data Part1" $ do
            v <- processInputPart1 <$> readInput day PuzzleExample1
            v @?= Right 5

        , testCase "test Example Data Part2" $ do
            v <- processInputPart2 <$> readInput day PuzzleExample1
            v @?= Right 12
         , testCase "test Data Part1" $ do
            v <- processInputPart1 <$> readInput day Puzzle
            v @?= Right 5306

        , testCase "test Data Part2" $ do
            v <- processInputPart2 <$> readInput day Puzzle
            v @?= Right 17787
    ]